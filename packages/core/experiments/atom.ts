...
class Queue<T = any> {
	...
	add(value: T) {
		this.tail =
			this.head === null ? (this.head = [value, null]) : (
...
const walk = (queues: Array<Queue>) => {
	for (let i = 0; i < queues.length; i++) {
		...
		if (queue.head !== null) {
			...
			// @ts-expect-error
			if (queue.async) {
				queue.handler(queue.next().value);
			} else {
				for (const value of queue) queue.handler(value);
			}
		}
	}
};
...
export interface Ctx {
	...
	subscribe<T>(action: Action<any, T>, cb: Fn<[T, AtomCache<Array<T>>]>): Unsubscribe,
...
export interface CtxSpy extends Ctx { spy<T>(atom: Atom<T>): T }

export interface Atom<State = any> { __reatom: AtomMeta<State> }
...
export type ActionResult<T> = T extends Action<any, infer Result> ? Result : never;
...
export const createCache: Fn<[AtomMeta], AtomCache> = (meta: AtomMeta): AtomCache => {
	return {
...
export const createContext = (): Ctx => {
	...
	const computersEnqueue = (cache: AtomCache) => {
		for (const childMeta of cache.children) {
			...
			if (childCache.actual) {
				patches!.set(
					childMeta,
					{ ...childCache, actual: false, cause: getCause(cache) },
				);

				if (childCache.listeners.size > 0) {
					Q.computers.add(childMeta);
				} else {
	...
	const actualizeParents = (patch: AtomCache) => {
		...
		if (
			patch.parents.length === 0 || patch.parents.some(
				(parentCache) =>
					!Object.is(
						parentCache.state,
				...
				Object.assign(
					...
					ctx,
					{
						spy: (atom: Atom) => {
							...
							isParentsChanged ||=
								patch.parents.length > parents.length || depPatch.meta !== parents[i++].meta;
	...
	const actualize: Ctx[typeof ACTUALIZE] = (meta, mutator) => {
		...
		try {
			mutator?.(patch);
			if (meta.computer !== null) {
				isParentsChanged = actualizeParents(patch);
			}
		} catch (err) {
		...
		if (cache !== undefined) {
			for (const parentCache of cache.parents) {
				...
				if (isStale(parentCache)) {
					unlinksHandler(parentCache);
				}
			}
		}

		if (hasHooks(patch.meta)) {
			Q.hooks.add([patch, false]);
		}
	};
	...
	const linksHandler = (patch: AtomCache) => {
		...
		for (const parentPatch of patch.parents) {
			if (isStale(parentPatch)) {
				linksHandler(parentPatch);
			}
			parentPatch.children.add(patch.meta);
		}

		if (hasHooks(patch.meta)) {
			Q.hooks.add([patch, patch.wasStale]);
		}
	};
	...
	const ctx: Ctx = {
		...
		subscribe(atom: Atom<any>, cb: Fn) {
			...
			const fn: typeof cb = (state, cache) =>
				(!atom.__reatom.isAction && Object.is(lastState, state)) || cb(
					(lastState = state),
					cache,
				);
			...
			return () => {
				if (subscribed) {
					...
					if ((listeners.size + children.size) > 1) {
						listeners.delete(fn);
					} else {
						ctx.run(() => actualize(meta, () => listeners.delete(fn)));
					}
				}
			};
		},
...
> = Merge<
	{
		[K in keyof Actions]: Actions[K] extends Fn<[State, ...infer Params]> ? Action<
			Params,
			State
		> : never;
	}
...
export const atom: {
	...
	<State>(initState: Fn<[CtxSpy], State>, options?: string | AtomOptions):
		& Atom<State>
		& ActionsReducers<State, {}>,
	<State>(initState: State, options?: string | AtomOptions):
		& Atom<State>
		& ActionsReducers<
			State,
...
export const action: {
	...
	const action = Object.assign(
		(ctx: Ctx, ...params: any[]) =>
			ctx.run(() => {
				const patch = ctx[ACTUALIZE](
					action.__reatom,
					(patch) => {
						patch.cause = ctx.top === null ? `external call` : getCause(ctx.top);
						patch.state =
							patch.state.concat([(fn as Fn)({ ...ctx, top: patch }, ...params)]);
					},
				);
